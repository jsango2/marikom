...
export async function getNovostById(id, preview) {
  ...
  novost: novost(id: $id, idType: ID) {
    ...
    `,
    {
      variables: {
        id: id,
        // U ovom slučaju, ID je numerički ID iz baze podataka
        // Možeš koristiti DATABASE_ID ili GLOBAL_ID, ovisno o tvojoj schemi
        // DATABASE_ID je najčešći za numeričke ID-eve
        // Ako ti ID dolazi kao string (npr. "cG9zdDoxMjM="), onda koristi GLOBAL_ID
        // Ako je ovo Custom Post Type u WPGraphQLu, provjeri dokumentaciju
        // za točan idType (npr. 'POST_ID', 'URI', 'SLUG' za post_name)
        idType: "DATABASE_ID",
        onlyEnabled: !preview,
        preview,
      },
